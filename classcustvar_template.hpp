...
private:
    ...
    void allocate (size_t newCapacity, size_t multiplier) {             // (to be allocated, multiplier)
        ...
        for (size_t i = 0; i < cd_capacity; i++) {cd_data[i].~T();}

        operator delete(cd_data);                                                         // free current block of memory

        cd_data = newblock;                                                 // assign current data* to new allocated block
        cd_capacity = ((newCapacity+cd_capacity)*1.5);                      // change the size to new block size
    }
    
    void allocate (size_t newCapacity) {                                    // (to be allocated)
        ...
        cd_data = newblock;
        cd_capacity = ((newCapacity+cd_capacity)*1.5);
    }
...
public:
    ...
    const T& peek () const {return cd_data[cd_size -1];}
